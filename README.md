...
- **Batch production** (`/graphic-machine`) — Turns a list of topics into a whole batch of graphics and/or carousels with per-channel captions and a spaced weekly schedule, behind ONE approval gate; renders everything, emits a single review file + manifest, then schedules the approved set across all platforms
- **Multi-client brand packs** (`tools/init-client.sh`) — Run the whole engine for any client from a per-client pack in `brand/clients/<slug>/` (brand colors, fonts, logo, GHL location + token). `/graphic-machine --client <slug> …` renders on that client's brand and publishes to their GHL sub-account. See [brand/clients/README.md](brand/clients/README.md)
- **Topic engine** (`/topic-queue`) — Generates a ranked backlog of concrete, on-brand content topics per client (from their pillars × an angle library), tracks what's been used so nothing repeats, and feeds `/graphic-machine --client <slug> --from-queue N` — so the pipeline never waits on "what should we post?"
- **Repurposing** (`/repurpose`) — Turns an existing reel/video (or just its key points) into a full content pack — reel cover (9:16) + carousel + single graphic + per-channel captions — then into the review→schedule flow. Transcribes video with `tools/transcribe.sh` and can grab a cover frame with `tools/grab-frame.sh`. One source in, everywhere out.
- **Feedback loop** (`/performance`) — Joins what was published (batch manifests: topic, format, pillar) to its GHL engagement, then reports the winning formats / pillars / platforms / topics (`tools/perf-report.mjs`). `/topic-queue` reads that back and weights the next backlog toward what's working — so the engine improves every cycle.
- **Omnichannel** (`/omnichannel`) — Turns one source into a full cross-channel pack — social graphics/carousel **+ email + landing page + SMS** — all on brand from one input (`tools/make-email.mjs`, `tools/make-landing.mjs`). Generates drafts to publish/send (it never auto-sends email or SMS). Be everywhere in as few clicks as possible.
